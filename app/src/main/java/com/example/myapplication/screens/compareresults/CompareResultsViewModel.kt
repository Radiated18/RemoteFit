package com.example.myapplication.screens.compareresults
...
class CompareResultsViewModel {
    ...
    private val _valueTypeComparacion = MutableLiveData<Int>()
    val valueTypeComparacion: LiveData<Int> = _valueTypeComparacion
    ...
    fun getTypeOfCompareResults(): Int {

        if (_weight.isInitialized && objetiveUserGet.isInitialized){
            if (abs( objetiveUserGet.value!!.weigh_register - _weight.value!!.toInt()) == 0){
                return 0
            }
            if (abs( objetiveUserGet.value!!.weigh_register - _weight.value!!.toInt()) in 1..3){
                return 1
            }
            if (abs( objetiveUserGet.value!!.weigh_register - _weight.value!!.toInt()) in 4..6){
                return 2
            }
            if (abs( objetiveUserGet.value!!.weigh_register - _weight.value!!.toInt()) >= 7){
                return 3
            }
        }
        return -1
    ...
    fun deleteFinalObjectivesForCurrentUser() {
        db.collection(Constants.OBJETIVE_RECORD_FINAL)
            .whereEqualTo("id_user", currentUser!!.id_user)
            .get()
            .addOnSuccessListener { documents ->
                for (document in documents) {
                    db.collection(Constants.OBJETIVE_RECORD_FINAL).document(document.id)
                        .delete()
                        .addOnSuccessListener {
